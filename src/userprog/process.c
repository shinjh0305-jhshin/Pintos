...
tid_t process_execute(const char *file_name)  //'echo x'가 도착한 상태
{
    ...
    struct list_elem *mov = NULL;
    struct thread *tmp = NULL;
    struct list_elem *begin = list_begin(&(thread->child_list));
    struct list_elem *end = list_end(&(thread->child_list));

    for (mov = begin; mov != end; mov = list_next(mov)) {
        tmp = list_entry(mov, struct thread, child);
        if (tmp->exitStatus == -1)
...
start_process(void *file_name_) /*'echo x'가 도착함*/
{
    ...
    sema_up(&thread->parent->sema_exec);
